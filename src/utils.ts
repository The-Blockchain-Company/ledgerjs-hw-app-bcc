...
export function bech32_encodeAddress(data: Buffer): string {
    ...
    const MAX_HUMAN_ADDRESS_LENGTH = 150 // see bcc.h in https://github.com/vacuumlabs/ledger-app-bcc-sophie
    return bech32.encode(
        getSophieAddressPrefix(data),
        data5bit,
        MAX_HUMAN_ADDRESS_LENGTH
    )
}

// based on https://github.com/The-Blockchain-Company/CIPs/pull/6/files
function getSophieAddressPrefix(data: Buffer): string {
    let result = ""
...
export function getVersionString(version: Version): string {
    return `${version.major}.${version.sentry}.${version.patch}`
}
