...
import { ASSET_GROUPS_MAX, MAX_ENTROPIC_SUPPLY_STR, TOKENS_IN_GROUP_MAX } from "./constants"
...
export function parseTransaction(tx: Transaction): ParsedTransaction {
    ...
    // fee
    const fee = parseUint64_str(tx.fee, { max: MAX_ENTROPIC_SUPPLY_STR }, InvalidDataReason.FEE_INVALID)
...
function parseWithdrawal(params: Withdrawal): ParsedWithdrawal {
    return {
        amount: parseUint64_str(params.amount, { max: MAX_ENTROPIC_SUPPLY_STR }, InvalidDataReason.WITHDRAWAL_INVALID_AMOUNT),
...
function parseTxOutput(
    ...
    const amount = parseUint64_str(output.amount, { max: MAX_ENTROPIC_SUPPLY_STR }, InvalidDataReason.OUTPUT_INVALID_AMOUNT)
